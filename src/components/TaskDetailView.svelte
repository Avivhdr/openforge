<script lang="ts">
  ...
  import { selectedTaskId, activeSessions, activeProjectId, error } from '../lib/stores'
  import { getWorktreeForTask, updateTaskStatus, deleteTask, getConfig } from '../lib/ipc'
  ...
  <header class="flex flex-col bg-base-200 border-b border-base-300 shrink-0">
    <div class="flex items-center gap-3 px-6 py-3.5">
      ...
      {#if actions.length > 0}
       ...
       {/if}
      <button
        class="btn btn-ghost btn-sm text-error shrink-0 hover:bg-error/10 transition-colors"
        title="Delete task"
